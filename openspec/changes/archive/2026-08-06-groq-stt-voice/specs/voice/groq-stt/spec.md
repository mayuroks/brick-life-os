...
The bridge SHALL require and validate the transcription provider API key at startup and SHALL return a clean, categorized client response for transcription outcomes (success / error / empty).

#### Scenario: Missing provider key
- **WHEN** the provider API key is not configured at startup
- **THEN** the bridge fails fast with a clear error identifying the missing configuration
